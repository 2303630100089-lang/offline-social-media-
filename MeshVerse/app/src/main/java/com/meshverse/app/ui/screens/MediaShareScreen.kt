...
fun MediaShareScreen(viewModel: MainViewModel) {
    ...
    Column(modifier = Modifier.fillMaxSize().padding(12.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
        ...
        Button(onClick = {
            ...
            transferManager.sendMedia(peerId, "sample.txt", "text/plain", sampleData)
            status = "Sent in ${transferManager.chunkCount(sampleData)} chunks with checksum ${transferManager.sha256(sampleData).take(12)}..."
        }) {
            Text("Send sample file")
        }
